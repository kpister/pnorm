...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, h, s, mask):
        ...
        e = torch.tanh(e)  # (bsz, max_src_len, hidden_size)
...
class GenerationProbabilty(nn.Module):
    ...
    def forward(self, h_star, s, x):
        ...
        p_gen = torch.sigmoid(p_gen.squeeze(1))  # (bsz, )
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, device, dropout_p=0):
        super(Encoder, self).__init__()

        self.device=device
        self.embedder = nn.Embedding(vocab_size, embedding_size, padding_idx=EOS_token)
    ...
    def forward(self, indices):
        ...
        lengths = torch.tensor([len(x) for x in indices], dtype=torch.long, device=self.device)
        indices_padded = pad_lists(indices, EOS_token, dtype=torch.long, device=self.device)
        ...
        h, _ = pad_packed_sequence(h, batch_first=True, padding_value=EOS_token)
...
class Decoder(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, use_hierarchical_attention, use_ptr_gen, device, max_decode_len=25, epsilon=1e-6, dropout_p=0):
        super(Decoder, self).__init__()
        self.device=device
        self.lemma_embedder = nn.Embedding(vocab_size, embedding_size, padding_idx=EOS_token)
        ...
        if use_hierarchical_attention:
            ...
            self.generator = nn.Linear(3*hidden_size, vocab_size)
        else:
            ...
            self.generator = nn.Linear(5*hidden_size, vocab_size)
    ...
    def forward(self, lemma_input_indices, h_l, h_tg, mask_l, mask_tg, decoder_initial, inputs=None, a_ls_true=None, p_gens_true=None):
        ...
        if inputs is not None:
            ...
            x = torch.ones(bsz, device=self.device, dtype=torch.long) * SOS_token # (bsz, )
            x = self.lemma_embedder(x)  # (bsz, embedding_size)

        s, c = decoder_initial  # (1, bsz, hidden_size), (1, bsz, hidden_size)

        p_ws = torch.ones(bsz, self.max_decode_len, VOCAB_SIZE, device=self.device) * self.epsilon  # (bsz, max_decode_len, char_vocab_size)
        a_ls = torch.zeros(bsz, self.max_decode_len, max_lemma_len, device=self.device)  # (bsz, max_decode_len, max_lemma_len)
        p_gens = torch.zeros(bsz, self.max_decode_len, device=self.device)  # (bsz, max_decode_len)
        ...
        for t in range(max_t):
            ...
            if self.use_ptr_gen:
                p_attn = torch.zeros(bsz, VOCAB_SIZE, device=self.device)  # (bsz, char_vocab_size)
...
class Model(nn.Module):
    def __init__(self, vocab_size, tag_size, embedding_size, hidden_size, use_hierarchical_attention, use_ptr_gen, device, dropout_p=0):
        super(Model, self).__init__()
        self.device=device
        self.lemma_encoder = Encoder(vocab_size, embedding_size, hidden_size, device, dropout_p=dropout_p)
        self.tag_encoder = Encoder(tag_size, embedding_size, hidden_size,device, dropout_p=dropout_p)
        self.bridge_h = nn.Linear(2*hidden_size, hidden_size)
        self.bridge_c = nn.Linear(2*hidden_size, hidden_size)
        self.decoder = Decoder(VOCAB_SIZE, embedding_size, hidden_size, use_hierarchical_attention, use_ptr_gen, device, dropout_p=dropout_p)
    ...
    def forward(self, lemma_indices, tag_indices, inflected_form_indices=None, a_ls_true=None, p_gens_true=None):
        ...
        lemma_indices_padded = pad_lists(lemma_indices, EOS_token, dtype=torch.long, device=self.device)
