...
class Engine:
    def __init__(self, args: Dict[str, Any]):
        ...
        if args['--load']:
            self.encoder.load_state_dict(torch.load(f"files/{args['--load']}.pkl", map_location=self.device))
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, vocab_size, dropout_p, device, max_length=MAX_LENGTH):
    ...
    def _forward(self, x:torch.Tensor, hidden:torch.Tensor, encoder_outputs:torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        output, hidden = self.gru(output, hidden.unsqueeze(0)) #ERROR
